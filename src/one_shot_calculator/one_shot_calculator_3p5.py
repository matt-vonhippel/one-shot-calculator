...
def one_shot_histogram(dataframe,low_CR,high_CR,one_shot_function):
    """Returns a histogram of the chance that an attack one-shots monsters in a given CR range.
       ...
       """
    return dataframe.loc[(dataframe["CR"]>=low_CR) & (dataframe["CR"]<=high_CR)].apply(one_shot_function,axis=1).round(3).hist(bins=list(map(lambda x: x/20,range(21))),density=True)